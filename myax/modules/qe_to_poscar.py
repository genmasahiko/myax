...
def read_structure(qe_input: Path, qe_output: Path | None):
    read, _ = _load_ase_io()
...
def write_poscar(qe_input: Path, qe_output: Path | None, output_path: Path) -> Path:
    _, write = _load_ase_io()
    atoms = read_structure(qe_input, qe_output)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert a Quantum ESPRESSO input/output structure to a POSCAR-format .vasp file."
    )
    parser.add_argument("qe_input", nargs="?", help="Quantum ESPRESSO input file.")
    parser.add_argument(
        "qe_output",
        nargs="?",
        help="Quantum ESPRESSO output file. If omitted, the input structure is used.",
    )
    parser.add_argument(
        "-o",
        "--output",
...
def main() -> None:
    ...
    interactive = args.qe_input is None and args.output is None

    qe_input = args.qe_input or input("QE input file: ").strip()
    qe_output = args.qe_output
    if qe_output is None and interactive:
        qe_output = input("QE output file (optional, press Enter to skip): ").strip()
    output_name = args.output or input("Output POSCAR file name: ").strip()

    if not qe_input or not output_name:
        raise SystemExit("QE input and output file name are required.")

    qe_input_path = Path(qe_input)
    qe_output_path = Path(qe_output) if qe_output else None
    ...
    if qe_output_path is not None and not qe_output_path.exists():
